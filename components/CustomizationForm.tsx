...
import type { Language, CustomizationDetails, ProductSize, ProductType, FormConfig, FormSectionKey, FormField, ProductColor } from '../types';
import { useCurrencyFormatter } from '../hooks/useCurrencyFormatter';

interface CustomizationFormProps {
  language: Language;
  details: CustomizationDetails;
  setDetails: React.Dispatch<React.SetStateAction<CustomizationDetails>>;
  allSizes: Record<ProductType, ProductSize[]>;
...
const CustomizationForm: React.FC<CustomizationFormProps> = ({ language, details, setDetails, allSizes, formConfig, colorPickerRef, availableColors, navigate }) => {
  ...
  const handleDimensionChange = (dimension: 'width' | 'height' | 'depth', value: string) => {
    ...
    if (!isNaN(numValue) && numValue >= 0) {
      setDetails(prev => ({ ...prev, [dimension]: numValue }));
    } else if (value === '') {
      setDetails(prev => ({ ...prev, [dimension]: 0 }));
    }
  };
  ...
  const handleQuantityChange = (newQuantity: number) => {
    if (newQuantity >= 1) {
      setDetails(prev => ({ ...prev, quantity: newQuantity }));
    }
  };
  ...
  const handleLogoUpload = (logoDataUrl: string) => {
    setDetails(prev => ({ ...prev, logoUrl: logoDataUrl }));
  };

  const handleLogoRemove = () => {
    setDetails(prev => ({ ...prev, logoUrl: null }));
  };
  ...
  const productSizes = allSizes[details.productType] || [];
      ...
      case 'dimensions':
        return (
          <div>
            ...
            <SizeSelector language={language} sizes={productSizes} selectedWidth={details.width} selectedHeight={details.height} onSizeSelect={handleSizeChange} />
            ...
            <div className={`grid gap-4 ${productHasDepth ? 'grid-cols-3' : 'grid-cols-2'}`}>
              <SizeInput label={t('width')} value={details.width} onChange={e => handleDimensionChange('width', e.target.value)} tooltipText={t(getTooltipKey('width'))} step={details.productType.includes('Card') ? 0.1 : 1} min={details.productType.includes('Card') ? 0.1 : 1} required={field.required} />
              <SizeInput label={t('height')} value={details.height} onChange={e => handleDimensionChange('height', e.target.value)} tooltipText={t(getTooltipKey('height'))} step={details.productType.includes('Card') ? 0.1 : 1} min={details.productType.includes('Card') ? 0.1 : 1} required={field.required} />
              {productHasDepth && <SizeInput label={details.productType.includes('ShoppingBag') ? t('gusset') : t('depth')} value={details.depth} onChange={e => handleDimensionChange('depth', e.target.value)} tooltipText={t(getTooltipKey('depth'))} min={1} required={field.required} />}
            </div>
          </div>
        );
      case 'color':
            ...
            {isCustomSize ? (
              ...
              <div className="space-y-2 text-center">
                 ...
                 <div className="flex justify-between items-baseline text-sm">
                   ...
                   <span className="font-medium text-gray-800">{formatCurrency(pricePerItem)}</span>
                 </div>
                 <div className="flex justify-between items-baseline pt-2">
                   <span className="text-gray-700 font-bold text-lg">{t('totalPrice')}:</span>
                   <span className="font-bold text-3xl text-primary">{formatCurrency(totalPrice)}</span>
                 </div>
              </div>
            ) : null}
          </div>
    ...
    quantityAndPrice: (
      <section aria-labelledby="quantity-heading">
          ...
          <div className="bg-slate-50 rounded-lg p-6 flex flex-col items-center gap-4">
            {formConfig.quantityAndPrice.fields.map(field => field.enabled ? <React.Fragment key={field.id}>{renderField(field)}</React.Fragment> : null)}
          </div>
      </section>
    ),
